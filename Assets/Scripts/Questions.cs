...
// public int DateAfter1(int days,int firstday,int desiredDay,int round,int afterDate,int month){
	...
	public void DateCount(){
		...
		if(ques.Contains("xxx")){
			...
			// int ans =WeekDayFormula(n,c,b,a);
			addQuestion.question =ques;
			addQuestion.answer=ans.ToString();
	...
	public int DateAfter1(int days,int firstday,int desiredDay,int round,int afterDate,int month){
		Debug.Log("Days:"+days+" firstDay "+firstday+" desiredDay "+desiredDay+" round "+round+" afterDate "+ afterDate+" Month "+ month);
		int date= afterDate;
		int counter=0;
		// int dayLeft = days-date;
		for(int i=date;i<=days;i++){
			// Debug.Log("Desired Day: "+WeekDays[desiredDay] );
			// Debug.Log("Weekdays "+WeekDays[i%7]);
			if(WeekDays[(i+firstday)%7]==WeekDays[desiredDay]){
...
//days,firstday,first,weekday,after
	...
	public void QuestionCreator(int number){
		...
		for(int i=0;i<number;i++){
			...
			// j=13;
			//2,,9(Question change),10,11(same as 9),
			//Solved-4,9,10;
			if(j==1){
				DayCount();
			}
